...
Meteor.methods({
    ...
    addNotice: function(idLesson, notice){
        ...
        if(!this.userId || !lesson || lesson.private.registered.indexOf(this.userId) == -1 || lesson.private.owner == this.userId){
            throw new Meteor.Error('not-authorized');
            return null;
        }
        ...
        if(alreadyNoticed){
            ...
            upLesson = Lessons.update({
                ...
                $set :{
                    'private.notices.$.comment' : notice['comment'],
                    'private.notices.$.grade' : notice['grade']
                }
            })
